...
test = open('all.txt').read().strip().split('\n')
...
for line in test:
	...
	if len(lineSplit)>7:
		term = lineSplit[0]
		coll = lineSplit[1]
		sentence = lineSplit[3].replace('"','')


		termC = lineSplit[4]
		collC = lineSplit[5]
		sentenceC = lineSplit[7].replace('"','')
...
#			SommeLn+= complexity(sentence,"EN")
			...
			if (sentence_length(sentence,stop_wordsEN)<=20 and sentence_length(sentence,stop_wordsEN )>=9) and (not anaphora_sentence(sentence,'EN')): 
				nn+=1
				te.append(term)
		...
		if sentenceC !='':
			...
			if (sentence_length(sentenceC,stop_wordsFR)<=20 and sentence_length(sentenceC,stop_wordsFR )>=8) and (not anaphora_sentence(sentenceC,'FR')): 
...
print "Nbr CRC Source :",n
print "Nbr CRC Cibles filtrés :", nn
...
print "Nbr CRC Cibles :",nC
print "Nbr CRC Cibles filtrés :", nnC
