...
def main():
    collocations = pickle.load(open('outCollocations/collocationLemmaFR.p','rb'))
    #defined_terms = open('CiblesTest.txt','r').read().lower().strip().split('\n')
    term = sys.argv[1]
    stop_words = open('stopWords/StopWordsFR.txt').read().strip().split('\n')
    ...
    #print nbr
    pickle.dump(couples, open('outCollocations/collocationLemmaPropFR.p', 'wb'))
