...
def main():
    collocationsSource = pickle.load(open('coll/collocationAllEN-VER.p','rb'))
    collocationsCible = pickle.load(open('coll/collocationAllFR-VER.p','rb'))

    traductionList = open('traduction-validee.txt').read().strip().split('\n')
    traductionList = dict([(li.split('\t')[0],li.split('\t')[2]) for li in traductionList]) 
    dico = [(t.split('\t')[0],t.split('\t')[1]) for t in open("EN-FR-ELRA").read().strip().split('\n')]
