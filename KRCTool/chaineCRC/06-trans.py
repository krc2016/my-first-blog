...
stop_wordsFR = open('stopWords/StopWordsFR.txt').read().strip().split('\n')
stop_wordsEN = open('stopWords/StopWordsEN.txt').read().strip().split('\n')
...
linesFR = list(set(open('corpus/vulcanoFR-POS.txt').read().strip().split('.	SENT	.')))

dictLemmFR = pickle.load(open('dictionnaries/dictSentenceLemmFR.p', 'rb'))
dictLemmEN = pickle.load(open('dictionnaries/dictSentenceLemmEN.p', 'rb'))

dictBilingue = pickle.load(open('dictionnaries/dictBilingue.p', 'rb'))
...
termsEN = open('terminology/terminoEN.txt').read().strip().split('\n')
termsFR = open('terminology/terminoFR.txt').read().strip().split('\n')
nbrColl = 0
nbrSource = 0
nbrCible = 0
...
print "----------------------"
Dict = [(t.split('\t')[0],t.split('\t')[1]) for t in open("dictionnaries/EN-FR-ELRA").read().strip().split('\n')]
...
#				if NbrTerm(sentenceS,termsEN)/float(len(sentenceS.split()))/NbrTerm(sentenceC,termsFR)/float(len(sentenceC.split()))>0.9:
				...
				(wS.lower() != termS and wS.lower() != collS) and (wC.lower()!=termC and wC.lower()!=collC) and (SequenceMatcher(None, wS.lower(), wC.lower()).ratio() >= 0.7)])>0) or (len([Ws for Ws in transTerm if Ws in Cibsent if ((Ws != termC and Ws != collC) and (Ws !=','))] )>0):
#					oo = True
...
#							ttt=True	
				...
				(wS.lower() != termS and wS.lower() != collS) and (wC.lower()!=termC and wC.lower()!=collC) and (SequenceMatcher(None, wS.lower(), wC.lower()).ratio() >= 0.7)]))+'\t')
					if (len([Ws for Ws in transTerm if Ws in Cibsent if ((Ws != termC and Ws != collC) and (Ws !=','))] )>0):
